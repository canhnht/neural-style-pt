...
def main():
    ...
    print('setup_gpu', dtype, multidevice, backward_device)
    ...
    style_image = preprocess(params.style_image, style_size).type(dtype)
    ...
    train_dataset = datasets.ImageFolder(
        params.dataset, transform=get_transform_preprocess(params.image_size))
    train_loader = DataLoader(
        train_dataset, batch_size=params.batch_size,
        shuffle=True, num_workers=8)

    transformer = TransformerNet().to(backward_device)
    ...
    optimizer, loopVal = setup_optimizer(transformer)
    while num_calls[0] <= loopVal:
        optimizer.step(feval)


# Configure the optimizer
def setup_optimizer(transformer):
    if params.optimizer == 'lbfgs':
        ...
        optimizer = optim.LBFGS(transformer.parameters(), **optim_state)
        loopVal = 1
...
def get_transform_preprocess(image_size):
...
def preprocess(image_name, image_size):
    ...
    transform = get_transform_preprocess(image_size)
